      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-20">
        <div className="max-w-[1800px] mx-auto px-4 md:px-6 py-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <h1 className="text-2xl font-serif font-bold text-gray-900 mb-1">Designer Directory</h1>
              <p className="text-sm text-gray-500">Browse designers, fashion brands, and emerging talent</p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button className="px-4 py-2 bg-gray-900 text-white rounded-lg text-sm font-medium hover:bg-gray-800 transition-colors flex items-center gap-2 shadow-sm">
            ...
            {kpis.map((kpi, i) => (
              <div key={i} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
                <div>
                  ...
                  <div className="text-2xl font-serif font-medium text-gray-900">{kpi.value}</div>
      ...
      <div className="max-w-[1800px] mx-auto px-4 md:px-6 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm mb-6 flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
                <select className="px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm text-gray-700 focus:outline-none cursor-pointer">
                ...
                <button 
                  ...
                  className={`px-3 py-2 rounded-lg text-sm font-medium border transition-colors flex items-center gap-2 ${filterEmerging ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-gray-200 text-gray-600'}`}
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {designers.map((designer) => (
                ...
                >
                  <div className={`h-24 ${designer.bgImage} relative`}>
                    <div className="absolute -bottom-8 left-6">
                      <div className="w-16 h-16 rounded-full bg-white p-1 shadow-sm">
                        <div className="w-full h-full rounded-full bg-gray-900 text-white flex items-center justify-center font-serif text-xl">
                          {designer.image}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="pt-10 px-6 pb-6">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h3 className="font-serif text-lg font-bold text-gray-900 group-hover:text-indigo-600 transition-colors">{designer.name}</h3>
                        <div className="text-xs text-gray-500 flex items-center gap-1">
                          <MapPin className="w-3 h-3" /> {designer.city}
                        </div>
                      </div>
                      <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wide border ${
                        designer.status === 'Active' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 
                        designer.status === 'In Review' ? 'bg-amber-50 text-amber-700 border-amber-100' : 
                    ...
                    <div className="text-sm text-gray-600 mb-4">{designer.category}</div>

                    {designer.tags.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-5">
                        {designer.tags.map(tag => (
                          <span key={tag} className="px-2 py-0.5 bg-gray-50 border border-gray-100 rounded text-[10px] font-medium text-gray-600">
                            {tag}
                          </span>
                        ))}
                      </div>
          ...
          <div className="w-full lg:w-80 space-y-6">
            <div className="bg-gradient-to-br from-violet-900 to-indigo-900 rounded-2xl p-6 text-white relative overflow-hidden shadow-lg">
              ...
              <div className="flex items-center gap-2 mb-4 relative z-10">
                ...
                <span className="text-xs font-bold text-violet-200 uppercase tracking-wider">Gemini Intelligence</span>
              </div>

              <div className="space-y-4 relative z-10 mb-6">
                <div className="bg-white/10 backdrop-blur-md rounded-xl p-3 border border-white/10">
                  <p className="text-sm leading-relaxed font-medium">
                    Paris-based luxury labels are trending <span className="text-green-300">+14%</span> in online engagement this week.
                  </p>
                </div>
                <div className="bg-white/10 backdrop-blur-md rounded-xl p-3 border border-white/10">
                  <p className="text-sm leading-relaxed font-medium">
              ...
              <div className="grid grid-cols-1 gap-2 relative z-10">
                ...
                <button className="w-full py-2 bg-indigo-800/50 border border-indigo-700/50 text-indigo-100 rounded-lg text-xs font-bold hover:bg-indigo-800 transition-colors">
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
              <h3 className="font-serif font-bold text-gray-900 mb-4">Quick Filters</h3>
